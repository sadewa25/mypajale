...
if($_SERVER['REQUEST_METHOD']=='POST') {
  ...
  $sql = "SELECT laporan_opt.id_laporan_opt, kabupaten.nama as 'kabupaten', kecamatan.nama as 'kecamatan', laporan_opt.judul_laporan, users.nama_lengkap, jenis_tanaman.nama_jenis_tanaman,
    ...
    INNER JOIN users ON users.id_users = laporan_opt.id_user WHERE laporan_opt.id_user = '$id_user'";
  ...
  if ($api == $apikey) {
    while($row = mysqli_fetch_array($res)){
      array_push($result, array('id_laporan_opt'=>$row[0], 'kabupaten'=>$row[1],'kecamatan'=>$row[2],
        'judul_laporan'=>$row[3],'nama_lengkap'=>$row[4],'nama_jenis_tanaman'=>$row[5],'gambar'=>$row[6]));
    }
  }
