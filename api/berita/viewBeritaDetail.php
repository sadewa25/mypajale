...
if($_SERVER['REQUEST_METHOD']=='POST') {
  ...
  $sql = "SELECT berita.id_berita, berita.judul_berita, berita.deskripsi_berita,berita.tgl_berita, berita.gambar_berita, users.nama_lengkap
        FROM berita
        INNER JOIN users ON users.id_users = berita.id_users where berita.id_berita = '$id_berita' ";
  ...
  if ($api == $apikey) {
    while($row = mysqli_fetch_array($res)){
      array_push($result, array(
        'id_berita'=>$row[0], 'judul_berita'=>$row[1],'deskripsi_berita'=>$row[2],'tgl_berita'=>$row[3],'gambar_berita'=>$row[4],'nama_lengkap'=>$row[5]
      ));
    }
  }
